    ...
    async _handleDeletedDownloads(currentDownloadIds) {
        ...
        for (const downloadId of deletedIds) {
            try {
                ...
                console.log(`[DownloadSyncService] Deleted download from database: ${downloadId}`);
            } catch (error) {
    ...
    async _writeChangesWithRetry(downloadId, changes) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                if (!entry) {
                    console.warn(`[DownloadSyncService] Download ${downloadId} no longer exists in memory, skipping write`);
                    return;
                }
                ...
                this._downloadRepository.updateProgress(downloadId, latestChanges);
                ...
                if (attempt < maxRetries - 1) {
                    ...
                    console.warn(`[DownloadSyncService] Retry ${attempt + 1}/${maxRetries} for download ${downloadId}`);
                } else {
                    // فشلت جميع المحاولات
                    this._stats.failedWrites++;
        ...
        for (const [downloadId, entry] of activeDownloads.entries()) {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    this._downloadRepository.updateProgress(downloadId, allData);
                    this._stats.successfulWrites++;
                    break; // نجاح
